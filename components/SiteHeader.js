...
const Navbar = () => {
    ...
    return (
        <nav className="mx-auto  sticky top-0 z-10 bg-[#d8c9c9] backdrop-filter backdrop-blur-[2px] bg-opacity-40 border-b border-gray-100 firefox:bg-opacity-90">
            <div className='text-[#FF0000] text-[1.5rem] text-center font-bold  bg-white'>
                <h1>THE SITE IS UNDER MAINTENANCE FOR ONE HOUR , SORRY FOR THE INCONVENIENCE CAUSED  </h1>
                <Link href='https://app.midhafin.com/s/mycourses'><h1 className='underline' >MEANWHILE PLEASE VISIT THIS LINK FOR ACCESSING COURSES</h1></Link>
            </div>
            <div className='max-w-full mx-3 '>
                <div className="flex items-center justify-between lg:w-[100%] h-[60px] mx-auto pr-4">
                    ...
                    <ul className="hidden md:flex  justify-around flex-nowrap w-[70%] lg:w-[60%] xl:w-[50%] ">
                        <NavItem to='https://app.midhafin.com/s/mycourses' name='Student Portal' />
                        {/* <NavItem to="/blog" name="Blogs" /> */}
                        <NavItem to="https://app.midhafin.com/s/store" name="Courses" />
                        <NavItem to="/reviews" name="Reviews" />
                        <NavItem to="/contact-us" name="Contact Us" />
                        <NavItem to="https://app.midhafin.com/s/authenicate" name="Login" />
                    </ul>
                    {/* Responsive */}
