...
export default function FeaturedImage({ post }) {
    ...
    // if(post.featuredImage) {
    //     let size = post.featuredImage.node.mediaDetails.sizes[4];
    //     img = {
    //         src: size.sourceUrl,
    //         width: size.width,
    //         height: size.height
    ...
    return (
        <Link href={`/${post.slug}`}>
            <Image src={img.src} width={img.width} height={img.height} alt={post.title} quality={100} className="w-[100%] object-cover rounded-xl" />
        </Link>
    );
}
