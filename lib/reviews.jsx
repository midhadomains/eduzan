    ...
    return (
        <>
                ...
                {data ? (
                    <ul className='grid sm:grid-cols-2 lg:grid-cols-3 gap-9 w-[90%] mx-auto'>
                        {data.map((review, index) => (
                            <li key={index} className='border-[3px] border-[#faf4f1] mt-[60px] pt-[60px] rounded-2xl p-4 bg-white relative backdrop-filter backdrop-blur-[2px] bg-opacity-20 shadow-inner'>
                                <div className='bg-grey-900  h-[100%] '>
                                    <div className=' items-center flex flex-col justify-between  h-[100%] text-center'>
                                        <Image width={120} height={120} src={review.image} alt={review.name} className='w-[100px] absolute mx-auto border-[3px] border-[#ffffff] -top-[65px] mx-auto rounded-[50%] border t-[40px] shadow-lg
                                        ?":' />
                                        <h1 className='text-center'>&quot; {`${review.excerpt}`}...<Link href={review.p_link} className='underline'>Read More</Link> &quot;</h1>
                                        <div>
                                            <h1 className='text-[#6A1B1B] text-lg font-semibold	'>{capitalizeWords(review.name)}</h1>
                                            <h1 className='text-amber-700 '>{review.course}</h1>
                                        </div>
                                        <p>{review.country}</p>
                                    </div>
                                </div>
            ...
            <div className="relative isolate overflow-hidden bg-white px-6 py-2 sm:py-2 lg:overflow-visible lg:px-0">
                ...
                <div className='p-5'>
                    <div className="w-[95%]">
                        <h1 className="font-bold text-[40px] text-center my-9 ">Don&apos;t believe us, Hear from our students</h1>
                    </div>
                    {data ? (
                        <ul className='grid sm:grid-cols-2 lg:grid-cols-3 gap-9 w-[90%] mx-auto'>
                            {data.map((review, index) => (
                                <li key={index} className='border-[3px] border-[#faf4f1] mt-[60px] rounded-2xl p-4 pt-9 bg-white relative backdrop-filter backdrop-blur-[1px] bg-opacity-[50px] shadow-inner'>
                                    ...
                                    <h1 className='text-amber-700  fixed top-1 right-1'>{review.course}</h1>

                                    <p className='fixed top-1 left-2'>{review.country}</p>
                                    <div className='bg-grey-900  h-[100%] '>
                                        <div className=' items-center flex flex-col justify-between  h-[100%] text-center my-1'>
                                            ...
                                            <Image width={120} height={120} src={review.image} alt={review.name} className='w-[100px] absolute mx-auto  border-[#ffffff] -top-[65px]  rounded-[50%] border t-[40px] shadow-lg?":' />
                                            <h1 className='text-center'>&quot; {`${review.excerpt}`}...<Link href={review.p_link} className='underline'>Read More</Link> &quot;</h1>
